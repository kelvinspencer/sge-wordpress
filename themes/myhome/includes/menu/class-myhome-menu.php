...
class My_Home_Menu {
    ...
    public function update_mmm_config() {
        ...
        $mmm_config['mh-primary_first_level_item_height'] = $options['mh-menu-height'];
        $mmm_config['mh-primary_first_level_item_align'] = $options['mh-menu-first-level-item-align'];
        $mmm_config['mh-primary_menu_first_level_link_font']['font_size'] = $options['mh-menu-first-level-item-size'];
        $mmm_config['mh-primary_menu_dropdown_link_font']['font_size'] = $options['mh-menu-dropdown-item-height'];
    ...
    public function set_logo( $nav_menu ) {
	    ...
	    $logo = '';
        if ( is_singular( 'estate' ) && $options['mh-estate_slider'] == 'single-estate-slider' ) {
        ...
        <span class="nav_logo">
            ...
            <?php if ( ! empty( $logo ) ) : ?>
                <a class="logo_link" href="<?php echo esc_url( site_url() ); ?>"
    ...
    public function add_elements( $items, $args ) {
        if ( ! isset( $args->container_class ) || ! ( strpos( $args->container_class, 'mh-primary' ) !== false ) ) {
            return $items;
        }
        $options = get_option( 'myhome_redux' );
        ...
        if ( function_exists( 'icl_get_languages' ) ) {
            ...
            foreach ( $languages as $lang ) :
                ...
                ?>
                <li>
                    <a href="<?php echo esc_url( $lang['url'] ); ?>"><?php echo esc_html( $lang['native_name'] ); ?></a>
                </li>
                <?php
                if ( $lang['active'] ) {
                    $lang_items = ob_get_clean() . $lang_items;
                } else {
                    $lang_items .= ob_get_clean();
                }
            endforeach;
            $items .= '<li><ul>' . $lang_items . '</ul></li>';
        }

        if ( ! isset( $options['mh-agent-submit_property'] ) || ! $options['mh-agent-submit_property'] ) {
            return $items;
        }
            ...
            <li id="mh-submit-button">
                <span class="item_link">
                    <span class="link_content">
                        <a id="myhome-submit-property" href="<?php echo esc_url( $panel_link ); ?>#submit-property"
