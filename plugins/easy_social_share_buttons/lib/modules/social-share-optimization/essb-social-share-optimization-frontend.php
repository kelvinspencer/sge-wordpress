	...
	private function loadPostSettings() {
		if (is_single () || is_page ()) {
			...
			if ($this->ogtags_active) {
				...
				$onpost_fb_authorship = get_post_meta (get_the_ID(), 'essb_post_og_author_of_post', true);
				
				if (!empty($onpost_fb_authorship)) {
					$this->fb_author_profile = get_post_meta ( get_the_ID(), 'essb_post_og_author_of_post', true);
				}
