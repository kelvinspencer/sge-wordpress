...
if ( ! class_exists( 'My_Home_Infobox' ) ) :

	class My_Home_Infobox extends WP_Widget {
		...
		<?php endif;
			...
			if ( ! empty( $text ) ) : ?>
				<div class="widget-infobox__text"><?php echo $text; ?></div>
			<?php endif;

			if ( $show_more_button ) : ?>
				<div class="widget-infobox__btn">
	                <a class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect" href="<?php echo esc_url( $link_url ); ?>" title="<?php echo esc_attr( $title ); ?>">
		...
		public function form( $instance ) {
			...
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id( 'show_more_button' ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_more_button' ) ); ?>" type="checkbox" value="1" <?php checked( $show_more_button, 1 ); ?> />
